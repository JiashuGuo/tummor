...
print(DS_interval)
hidden_nodes = args.hidden_nodes[0][0]      #
print(hidden_nodes)
...
acc_savename=str(num_classes)+'_normal_acc'+'.csv'
perp_savename=str(num_classes)+'_normal_perp'+'.csv'
numcurves_savename=str(num_classes)+'_normal_numOfcurves'+'.csv'
...
#                        plt.legend(loc='center right')
      ...
      #%%
        ...
        loss, accuracy, perp =new_model.evaluate(new_testX, new_testY, batch_size=batch_size, verbose=0)

        testPredict = new_model.predict(new_testX, batch_size=batch_size)
        a = numOfcurves(testPredict)
        test_no_curves= np.mean(a)
        print('test accuracy = '+ str(accuracy ) +' test perplexity = '+ str(perp) + ' test num of curves = '+str(test_no_curves ))


        Acc.append(accuracy)
        Perp.append(perp)
        Num.append(test_no_curves)
    with open(acc_savename, mode='a') as wf1:
        writer = csv.writer(wf1)
    ...
    #testY  = testY .as
acc_figsave=str(num_classes)+'normal_acc'+'.pdf'
perp_figsave=str(num_classes)+'normal_perp'+'.pdf'
numcurves_figsave=str(num_classes)+'normal_num'+'.pdf'
